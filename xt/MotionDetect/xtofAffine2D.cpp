	...
	double param1, double param2)
{
	...
	//CV_Assert(0);提示错误，1正常运行
	CV_Assert(count >= 0 && to.checkVector(2, CV_32F) == count);
	_out.create(2, 3, CV_64F);//需要定义为mat类
	Mat out = _out.getMat();
	...
	double confidence, int maxIters)
{
	...
	if (count < modelPoints)//modelPoints = 3，即点数最少为3
		return false;
	...
	for (iter = 0; iter < niters; iter++)
	{
		...
		nmodels = runKernel(ms1, ms2, models);//nmodels结算出模型1，未解算出模型0
		if (nmodels <= 0)
			continue;
		...
		for (i = 0; i < nmodels; i++)//可以尝试多生成几个模型，然后选出最好的
		{
			...
			goodCount = findInliers(m1, m2, &model_i, err, tmask, reprojThreshold);//err误差，tmask是否为内点的标志变量
	...
	if (maxGoodCount > 3)
	{
		...
		myGetAffineTransform(myMs1, myMs2, model, maxGoodCount);
	}
